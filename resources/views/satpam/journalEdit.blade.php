...
<div class="max-w-4xl mx-auto">
    ...
    {{-- Page Header --}}
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-800">Edit Journal</h1>
    ...
    <form action="{{ route('satpam.journal.update', $journal->id) }}" method="POST" enctype="multipart/form-data" id="journal-form">
        @csrf
        @method('PUT')
                        ...
                        <div class="relative">
                            <select name="lokasi_id" id="lokasi_id"
                                ...
                                @foreach($locations as $location)
                                    <option value="{{ $location->id }}" {{ old('lokasi_id', $journal->lokasi_id) == $location->id ? 'selected' : '' }}>
                        ...
                        <div class="relative">
                            <select name="shift_id" id="shift_id"
                                ...
                                @foreach($shifts as $shift)
                                    <option value="{{ $shift->id }}" {{ old('shift_id', $journal->shift_id) == $shift->id ? 'selected' : '' }}>
                                        {{ $shift->nama_shift }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="relative">
                            <select name="next_shift" id="next_shift"
                                ...
                                @foreach($groups as $group)
                                    <option value="{{ $group->id }}" {{ old('next_shift', $journal->next_shift) == $group->id ? 'selected' : '' }}>
                                        {{ $group->nama_grup }}
                                    </option>
                                @endforeach
                            </select>
            ...
            <div class="px-6 py-5 space-y-5">
                ...
                <div>
                    ...
                    <textarea name="laporan_kegiatan" id="laporan_kegiatan" rows="4"
                        ...
                        {{ $errors->has('laporan_kegiatan') ? 'border-red-400 bg-red-50' : '' }}">{{ old('laporan_kegiatan', $journal->laporan_kegiatan) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea name="kejadian_temuan" id="kejadian_temuan" rows="3"
                        ...
                        class="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2.5 text-sm text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition resize-none">{{ old('kejadian_temuan', $journal->kejadian_temuan) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="lembur" id="lembur"
                        required
                        value="{{ old('lembur', $journal->lembur) }}"
                ...
                <div>
                    ...
                    <textarea name="proyek_vendor" id="proyek_vendor" rows="3"
                        ...
                        class="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2.5 text-sm text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition resize-none">{{ old('proyek_vendor', $journal->proyek_vendor) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea name="barang_inven" id="barang_inven" rows="3"
                        ...
                        class="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2.5 text-sm text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition resize-none">{{ old('barang_inven', $journal->barang_inven) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea name="info_tambahan" id="info_tambahan" rows="3"
                        ...
                        class="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2.5 text-sm text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition resize-none">{{ old('info_tambahan', $journal->info_tambahan) }}</textarea>
                </div>
            </div>
        </div>
